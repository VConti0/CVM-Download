...
dest_folder = os.path.join(base_folder, "FRE")
...
# ----------------------------
for ano in anos:
    zip_filename = f"fre_cia_aberta_{ano}.zip"
    ...
    try:
        ...
        print(f"⚙️ {zip_filename} extraído.")
...
for ano in anos:
    padrao = os.path.join(dest_folder, f"fre_cia_aberta_auditor_{ano}.csv")
    ...
    for csv_file in arquivos:
        try:
            ...
            if "DATA_INICIO_CONTRATACAO" in df.columns:
                df["DATA_INICIO_CONTRATACAO"] = pd.to_datetime(df["DATA_INICIO_CONTRATACAO"], errors="coerce")
            if "CNPJ_COMPANHIA" in df.columns:
                df["CNPJ_COMPANHIA"] = df["CNPJ_COMPANHIA"].astype(str).str.strip()

            dfs.append(df)
            print(f"📁 FRE do ano {ano} carregado com sucesso.")
...
if dfs:
    ...
    # Ordenação final: CNPJ_CIA crescente, Data_Referencia decrescente, Data_Inicio_Contratacao decrescente
    combined_df = combined_df.sort_values(
        by=["CNPJ_COMPANHIA", "DATA_REFERENCIA", "DATA_INICIO_CONTRATACAO"],
        ascending=[True, False, False]
    )

    output_file = os.path.join(dest_folder, "fre_cia_aberta_auditor_combinado.csv")
    combined_df.to_csv(output_file, index=False, sep=";", encoding="ISO-8859-1", decimal=",")
    print(f"\nFRE combinado e ordenado salvo em: {output_file}")
else:
    print("Nenhum FRE CSV foi encontrado para processar.")
# ----------------------------
